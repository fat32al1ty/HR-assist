...
**AI-помощник для поиска по hh.ru.**

> Upload a resume — get a structured candidate profile, a ranked shortlist of real vacancies sourced from hh.ru, a per-vacancy "why shown" explanation, and a gap analysis of the skills you are missing. Every match links back to the original posting on hh.ru — we do not republish vacancy content; we are an AI-assisted search layer over the hh.ru API. Feedback on each vacancy (shortlist / blacklist / like / dislike) is time-decayed and fed back into ranking.
...
HR Assist is an AI-assisted search layer over hh.ru, built for jobseekers. A user uploads a PDF or DOCX resume; the platform extracts text, runs a structured LLM analysis, infers an approximate seniority grade, pulls live vacancies from the hh.ru API, embeds both sides into Qdrant, and ranks the results with a multi-stage semantic matcher. Every recommended vacancy is rendered with a visible link back to hh.ru. Archived postings are pruned on read (top-N freshness check) and on a nightly sweep. We do not maintain a derivative database of hh.ru content; we cache enough metadata to rank, then send the user to the canonical posting.

For every vacancy the user sees why it was recommended and which skills are missing to reach the bar. Shortlist, blacklist, like and dislike signals are weighted by time decay and feed back into future ranking.
...
- `v0.22.1` — Patch on top of v0.22 after a prod diagnostic: (1) HH-pagination cap from 90 → 11 because HH refuses `(page+1)*per_page > 2000` and the warmup worker was burning ~185 bad 400-requests per hour on every retry; (2) new `sweep_stale_segment_warmup_jobs` with a long 30-min timeout so a worker restart mid-crawl no longer leaves segment_warmup rows stuck in `running` forever (which blocked the unique partial index from re-enqueueing the same segment). Bonus: stabilised a flaky v0.22 test that iterated the whole vacancies table at `time.sleep(0.5)` per row → 11-min CI runs; now patches `time.sleep` and caps limit, full suite back to 70s.
- `v0.22.0` — Phase 6 step 4 (final): freshness + ToS compliance. Closes the zombie-vacancy hole and re-frames the product against hh.ru ToS §3.11. **F1 (on-read freshness):** the instant endpoint hits `GET https://api.hh.ru/vacancies/{id}` for the top-N (default 20) matches in parallel via `httpx.AsyncClient`; an `archived: true` flag or a 404 soft-deletes the row (`status='archived'`, `archived_at=now()`) and excludes it from the response. Best-effort, never 500s the request. **F2 (nightly sweep):** `vacancy_warmup_worker._run_freshness_sweep_if_due` re-checks up to 500 oldest-checked rows per day (~4 minutes per cycle, polite 0.5s delay between HH calls). **F3 (framing):** every VacancyCard now renders a visible source-host button (e.g. `hh.ru ↗`) with bordered + accent-filled styling instead of a subtle "Источник →"; README + README.ru + PRIVACY rewritten as "AI-assisted search layer over hh.ru", not "our vacancy database". Migration `0038_vacancy_freshness` adds `last_freshness_check`, `archived_at`, `shown_count` columns. Eval: 20 new tests in `test_vacancy_freshness.py`.
- `v0.21.0` — Phase 6 step 3: lazy segment populate. When the search button hits a cold segment (no matches in the local pool for the resume's role/seniority/top-3-domains combo), the instant endpoint derives a `segment_key` and enqueues a `segment_warmup` job; the background `vacancy_warmup_worker` drains pending segment-jobs each cycle, runs HH crawl + LLM analyze inside a new `system_budget_scope` (separate from per-user daily budget) with per-query 429 backoff. Dedup via unique partial index on `(segment_key) WHERE status IN ('queued','running')` — two users hitting the same segment within a minute = one job. Daily cap of 100 segments/day caps cost at ~$0.45/day worst case. Recovery on restart: the worker self-heals by querying the DB each cycle, no in-memory queue. Frontend: an honest "Прогрев индекса под твою роль · 5–10 минут" banner replaces the bland skeleton when `segment_warming=True`. Migration `0037_segment_warmup_jobs`: +columns `job_type`, `segment_key`, `notify_user_id`. Eval: 13 new tests (segment-key purity, dedup, cold-instant enqueue, daily cap, recovery, system_budget_scope isolation).
- `v0.20.0` — Phase 6 step 2: decouple deep-scan from the search button. `refreshVacancyIndex` no longer calls `POST /recommend/start` — refresh = single instant call against the local prefetched index in <1s, no HH crawl, no LLM analyze in the request thread. The Stage 2 polling loop, merge-by-id, and live OpenAI cost ticker are gone from the hot path. `/recommend/start` stays for admin/debug. Industry pattern: ingestion async, query = local index only (LinkedIn Galene, Indeed, hh.ru). Acceptance: p95 latency «Подбор» ≤1s, OpenAI cost/day drops ~95%, v0.19 instant-snapshot persistence guarantees refresh parity. Cold-segment lazy populate ships in v0.21.
- `v0.19.0` — Phase 6 step 1: persist Stage 1 instant matches as a `completed` row in `recommendation_jobs` so refresh restores the same view via `/recommend/latest`. Without this, refresh pulled the older deep_scan snapshot and a high-quality instant result regressed back to the worse Stage 2 one. New `record_instant_recommendation_snapshot` helper writes synchronously after the instant endpoint's success path; no migration. Eval: 2 new tests (refresh-after-instant returns same matches; empty instant also persists). Sets up the decouple in v0.20.
- `v0.18.0` — Drop the `/audit` feature entirely. The implementation conflated salary baseline, skill-gap tokens, and onboarding questions in a single page that produced false signals on non-engineering resumes (PM with 13 years showed 170k median; "top market skills" listed Kafka/Kubernetes for a product role; onboarding questions never rendered). Removing buys back UX honesty until we redesign per role-family. Removed: `/audit` route + components, `resume_audit` service + cache, `onboarding_questions` service + YAML, audit admin endpoints, eval fixtures. Kept: `salary_predictor` / `salary_baseline` / `track_classifier` / `llm_cost_accounting` (still used by matcher and `vacancy_strategy`); `resume_audits` / `resume_clarifications` DB tables (orphan rows left, no destructive migration).
- `v0.17.0` — Auto-pin rollback + multi-facet discovery + feedback loop. Production raised v0.16.0's flaw: auto-pin (writing `analysis.target_role/domains` to `User.preferred_*` on first save) corrupted persistence and broke matching for one of our users (0 matches after pre-filter dropped 205/208). **Tier 1 backend fix**: `_validate_titles` now rejects items <4 chars and a generic-stems blocklist (`product`, `manager`, `lead`, `head`, `director`, `specialist`, `engineer`, `developer`, `analyst` standalone); `_build_discovery_query` no longer truncates `preferred_titles[:2]`; `discovery_query_noisy_pref` warning logs already-affected users. Frontend `handleSaveAndRecommend` no longer unions auto-detected with localRoles; pills demoted into a `<details>` "Дополнительные фильтры (необязательно)"; new "Подбираем для тебя: {role} · {seniority} · {top 3 domains}" read-only summary above match cards links to `/audit`. **Tier 2 multi-facet**: `_build_deep_scan_queries` adds 3 independent facet variants (role_family alone / top hard skills only / role_family + top skill combo), deduped + capped at 6, `multi_facet_queries_generated` counter in telemetry. **Tier 3 feedback loop**: `feedback_signal_extractor.get_negative_term_set()` aggregates tokens from up to 30 recent dislikes (≤30d), subtracts resume hard skills, ScoringStage applies −0.02 per overlap (cap −0.06) — soft penalty, never drops. Gated on `settings.preference_decay_enabled`. Magnitudes calibrated below DOMAIN_PREFERENCE_BOOST (+0.03) and TITLE_BOOST_PARTIAL (+0.05).
- `v0.16.0` — Editable role + domain pills in the sidebar: two pill groups (Роли up to 5, Домены up to 3) with auto/pinned visual states, inline typeahead backed by `GET /api/users/preferences/suggestions?type=role|domain&q=&limit=` (5-min cache, queries against `vacancy_profiles`). New `User.preferred_domains` column (migration 0035) wires through `PATCH /api/users/me/preferences`. `_build_discovery_query` honours both `preferred_titles` and `preferred_domains` overrides; matcher applies a soft `+0.03` boost (counter `domain_preference_boost_applied` in telemetry) when a vacancy's domains intersect the user's preferences — never drops a vacancy. "Save and refresh" button triggers an instant-first refresh with the new filters. Eval: 16 new tests (PATCH cap/clear/persist + suggestions sort/prefix/auth + discovery query merge + matcher soft-boost A/B). Designer added 16 semantic tokens (`pill-auto-*`, `pill-pinned-*`, `combobox-*`, `unsaved-indicator-fg`) + `pill-in`/`pill-out` motion under `prefers-reduced-motion` guard.
- `v0.15.0` — Instant-first matching UX: new sync `POST /api/vacancies/recommend/instant/{resume_id}` returns matches from the prefetched index in <5s with a `prefetch_empty` skeleton hint when the index is cold; the heavy deep scan runs in the background without a blocking spinner, results merged on completion. Server-side `recommendation_job_timeout_seconds` lowered 420→180s; when the inner 150s runtime budget elapses the job ends as `completed` with `metrics.partial=true`, frontend renders an info banner instead of an error. Frontend defaults flipped to `use_prefetched_index=true, discover_count=40` (was `false/100`). New `sweep_stale_running_jobs()` janitor in `vacancy_warmup_worker` flips orphaned `running` rows to `failed` once per cycle. Eval: 11 new tests (instant happy/cold/404/no-HH-call + partial flag round-trip + zombie sweeper).
- `v0.14.0` — Per-vacancy strategy + cover letter draft + recommendation corrections: new `/strategy/{resume_id}/{vacancy_id}` page renders 3 editorial blocks — `match_highlights` (top-3 experience entries that match), `gap_mitigations` (top-2 missing requirements + how to close them), `cover_letter_draft` (≤ 1200 chars). Backend `vacancy_strategy.compute_strategy()` runs through an LLM path (PII-scrubbed input, regex-sanitized output) or template path; selection driven by a $0.05/DAU/day cost-cap shared with `/audit` via `llm_cost_accounting.daily_user_llm_cost_usd()`. 30-day cache in `vacancy_strategies` (migration 0033), 2/hour rate limit. New `POST /api/recommendation-corrections` (migration 0034) records user-issued corrections of strategy blocks for a future ranker hook. Frontend "Стратегия" button on each vacancy card; `sessionStorage` flag drives apply-after-strategy-view detection. 6 new telemetry events, 5 new design tokens. Eval: 54 new tests (template mode 7 + endpoint 6 + corrections 7 + cost accounting 4 + PII hard guard 30 parametrized) — 638/638 backend tests green; 0 PII leaks across 30 hand-built email/phone/name test pairs.
