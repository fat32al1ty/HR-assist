# HR-Assist — AI-помощник для поиска по hh.ru

> Загружаешь резюме — получаешь отранжированный список релевантных вакансий с hh.ru, честный разбор «чего хватает» и «чего нет», и воронку откликов без ручной рутины. Каждая карточка ведёт на оригинальную вакансию на hh.ru — мы не отдельный каталог, а AI-слой поиска поверх hh.ru API. Архивные вакансии оперативно убираем (on-read проверка + nightly sweep).
...
- `v0.14.0` — Phase 5.2 (Per-vacancy strategy + cover letter + recommendation corrections): новая страница `/strategy/{resume_id}/{vacancy_id}` — 3 editorial-блока: `match_highlights` (top-3 опыта релевантного вакансии), `gap_mitigations` (top-2 требований, которых нет, + как закрыть), `cover_letter_draft` (≤ 1200 chars). Backend `vacancy_strategy.compute_strategy()` — два пути: LLM (PII-scrubbed input + regex-sanitized output) и template (rule-based skill overlap + skeleton). Cost-cap $0.05/DAU/день шарится с `/audit` через `llm_cost_accounting.daily_user_llm_cost_usd()`; при превышении — fallback на template. Cache 30 days в `vacancy_strategies` (миграция 0033), rate limit 2/час. Новый `POST /api/recommendation-corrections` (миграция 0034) — юзер помечает «не я / не правда» на каждой карточке, данные летят в feedback loop для будущего ranker'а. Frontend: «Стратегия» кнопка на карточке вакансии; sessionStorage-flag для apply-after-strategy-view detection. 6 новых telemetry-событий (`strategy_view`, `strategy_match_highlight_corrected`, `strategy_gap_mitigation_corrected`, `cover_letter_copied`, `cover_letter_edited`, `apply_after_strategy_view`), 5 новых design-tokens. 54 новых теста (template 7 + endpoint 6 + corrections 7 + cost accounting 4 + PII hard guard 30 параметризованных), 638/638 backend tests green; 0 PII leaks на 30 тестовых пар email/phone/имя в template mode.
