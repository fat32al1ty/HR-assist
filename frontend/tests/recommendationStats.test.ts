...
test('headline for 0 fresh matches nudges the user about cadence', () => {
  ...
  assert.ok(message.includes('Рынок пока без свежих'));
  assert.ok(message.includes('6-12 часов'));
  assert.ok(!message.includes('Найдено совпадений'));
});

test('headline for 1-3 fresh matches uses "Свежего с прошлого раза" phrasing', () => {
  assert.equal(formatRecommendationHeadline(1), 'Свежего с прошлого раза: 1.');
  assert.equal(formatRecommendationHeadline(3), 'Свежего с прошлого раза: 3.');
});

test('headline for 5+ fresh matches uses the same count-based phrasing', () => {
  assert.equal(formatRecommendationHeadline(5), 'Свежего с прошлого раза: 5.');
  assert.equal(formatRecommendationHeadline(42), 'Свежего с прошлого раза: 42.');
});
...
test('metrics info hides the "already in index" jargon in favor of honest copy', () => {
  ...
  assert.ok(info.includes('из них 18 новых'));
